...
@triton.jit
def block_int8_weight_quant_kernel(x_ptr, y_ptr, s_ptr, M, N, BLOCK_SIZE: tl.constexpr):
...
# quant to block int8
def block_int8_weight_quant(x: torch.Tensor, block_size: int = 128) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    block_int8_weight_quant_kernel[grid](x, y, s, M, N, BLOCK_SIZE=block_size)
    return y, s
...
def get_w8a8_block_int8_configs(
    ...
    config_file_path = Path(__file__).parent / "configs" / json_file_name
