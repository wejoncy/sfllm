...
#pragma unroll 1
  for (int64_t idx = thread_idx; idx < d % (stride * vec_size); idx += stride) {
    vec_t<float, 1> x_vec, y_vec, out_vec;
    x_vec.cast_load(input + offset + remaining_offset + idx);
    y_vec.cast_load(input + offset + remaining_offset + d + idx);
    out_vec[0] = Activation(x_vec[0]) * y_vec[0];
    out_vec.cast_store(out + token_idx * d + remaining_offset + idx);
  }
